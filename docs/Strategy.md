...
- **Roadmap (post-v1.0), re-ordered by payoff:** the Audit Narrator (a grounded summarizer over the system's own hash-chained audit log — turns a *provable* log into a *reviewable* one, at low marginal cost since it reuses the log already emitted) first; then ML alert auto-closure QA (PP-13 — re-adjudicate a sample of auto-closed alerts, "AI overseeing AI") and vendor reconciliation (#8); then tokenized-commodity issuance tracing (PP-14 — follow gas-funding chains from bespoke oil/gold/fiat-peg token deployers), the multilingual OSINT verifier (#5), and LE-request/MLAT routing (#4/#7).
